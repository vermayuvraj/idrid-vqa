...
fine-tuning raises exact-match grading accuracy from \textbf{31.1\% to 66.0\%}
and quadratic weighted kappa (QWK) from \textbf{0.00 to 0.76} (substantial
agreement), roughly triples ROUGE-L explanation overlap ($0.08\!\rightarrow\!0.25$),
and nearly halves inference latency, while producing structured clinical reports at
zero marginal per-image cost. The entire fine-tune runs in about 90 minutes within
3.52\,GB of VRAM, and we find input resolution to be the single most important
factor for grading accuracy. We release the dataset, adapter weights, and code to support
...
longer side is at most 768 pixels ($\approx$486 visual tokens); this resolution
is the single most important hyperparameter we found (Section~\ref{sec:analysis}),
as smaller inputs render microaneurysms invisible. We train for 4 epochs with the
8-bit AdamW optimiser, a linear schedule, and learning rate $2{\times}10^{-4}$.
Only $28.9$M parameters ($1.45\%$ of the model) are trainable. In our runs the full
4-epoch fine-tune (824 optimiser steps over the 1{,}652 training QA pairs) completes
in \textbf{about 90 minutes} on a single laptop RTX 4050 and peaks at
\textbf{3.52\,GB} of VRAM---well inside the 5.5\,GB budget, and essentially
unchanged from the 512-pixel setting thanks to gradient checkpointing---confirming
that explainable DR grading can be trained on commodity hardware. Because \texttt{torch.compile}'s inductor backend requires a system C
...
\textbf{66.0\%} and, more tellingly, QWK from 0.00 to \textbf{0.76}. The zero-shot
base model is effectively non-discriminative: it emits verbose, generic
descriptions and gravitates to the majority ``moderate'' class, so its grades carry
no ordinal signal (QWK $\approx 0$). The fine-tuned model instead produces
calibrated 0--4 grades that correlate substantially with expert labels. Because
grading is ordinal, QWK---which penalises a No-DR/Proliferative confusion far more
than an adjacent error---is the clinically meaningful metric, and 0.76 (upper
``substantial'' agreement, Landis--Koch) is the headline result. For context, our
DR-only accuracy (66.0\%) is in the range of the top IDRiD-challenge entries, which
report \emph{joint} DR+DME accuracies of 63--65\%~\citep{li2019canet}; joint grading
is a stricter task, and specialised DR-only classifiers trained on far larger
corpora (APTOS, EyePACS) reach higher QWK---but without producing explanations or
running on consumer hardware.

\paragraph{Resolution is the key lever.} Our largest single gain came not from
architecture or optimisation but from \emph{input resolution}. Raising fundus
images from 512 to 768 pixels ($216\!\rightarrow\!486$ visual tokens) lifted
accuracy by 11.6 points ($54.4\!\rightarrow\!66.0\%$) and QWK by 0.14
($0.62\!\rightarrow\!0.76$), at essentially no extra VRAM---gradient checkpointing
keeps activations small---and only a modest latency cost. Microaneurysms, the
lesions that define early DR, span only a handful of pixels in the original image
and are effectively erased by aggressive downsampling. We therefore recommend
practitioners budget resolution before other hyperparameters.

\paragraph{Explanations.} ROUGE-L roughly triples ($0.08\!\rightarrow\!0.25$) and
BERTScore rises from $0.00$ to $0.31$. Qualitatively, the base model hallucinates
...
``cotton-wool spots alongside extensive hemorrhages and microaneurysms indicate
significant retinal ischemia, classifying this as severe NPDR.'' It is also nearly
$2\times$ faster (5.6 vs.\ 10.6\,s/image) because it stops at a structured answer
instead of running to the token limit.
